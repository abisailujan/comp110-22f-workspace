...
# Print both items on the same line, use f string
...
# which means we cannot reassign items (append/pop)
# goat[0] = "LBJ" 

# We can 'invent' our own type with a type alias
Player = tuple[str, int] 

# once we alias the type, we can create variables of that type
unc_poy: Player = ("Baycot", 5)

# In a strange world where jersey number changes
unc_poy = (unc_poy[0], unc_poy[1] + 1) # This creates a whole new tuple


# A range is another common sequence type 
zero_to_nine: range = range(0, 10, 1) #middle argument is end that is not inclusive, third agument is a uniform step of one
...
for i in range(len(names)): # len(names) is stop argument
    print(f"{i}. {names[i]}") # prints each index and element, excluding last element
